...
`kbuild` is the shared build and repo orchestration implementation for
ktools-style projects. `README.md` is for operators and implementers using the
tool; this file is for developers changing the `kbuild` codebase itself.
...
2. Read `docs/index.md` and `docs/kbuild.md` before changing CLI behavior,
   config handling, or repo bootstrapping.
...
- Keep user-facing usage guidance in `README.md` and `docs/`. Put repo-working
...
- Treat `templates/` as product code. Template changes alter newly scaffolded
  repos, so verify placeholder names, generated paths, and accompanying docs.
- `--kbuild-init` currently scaffolds CMake-based repos only. Do not widen docs
  or assumptions beyond that unless the implementation changes end to end.

## Cross-Repo Context

- If a task crosses into another ktools workspace or generated repo, read that
