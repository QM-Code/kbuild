...
def _load_swift_config(repo_root: str) -> tuple[str, dict[str, tuple[str, str]]]:
    raw = config_ops.load_effective_kbuild_payload(repo_root, require_local=True)
    swift_raw = raw.get("swift")
    if not isinstance(swift_raw, dict):
        errors.die("kbuild config key 'swift' must be an object")

    allowed_swift = {"package_path", "demo_products"}
    ...
    return package_path, demo_products
...
def build_repo(
    *,
    ...
    package_path, demo_products = _load_swift_config(repo_root)
